...
.guide summary{{cursor:pointer;font-size:17px;font-weight:650;list-style-position:inside}} .guide[open] summary{{margin-bottom:18px}} .guide h3{{font-size:15px;margin:16px 0 6px}} .guide p{{margin:0 0 10px}}
...
def cabinet(request: Request):
    ...
    guide = """<details class='card guide'><summary>Как подключиться</summary><h3>На сайте</h3><p>Под этой инструкцией найдите поле <b>«Название устройства»</b>. Напишите туда любое понятное название, например <b>Вася Пупкин</b>, и нажмите красную кнопку <b>«Добавить устройство»</b>. Ниже появится карточка устройства с кнопками.</p><h3>Если сайт открыт на подключаемом телефоне или компьютере</h3><p>В карточке устройства нажмите <b>«Файл»</b>. Браузер скачает файл настроек. Откройте список загрузок, нажмите на скачанный файл и выберите <b>AmneziaWG</b>. В приложении подтвердите добавление подключения.</p><h3>Если сайт открыт на другом устройстве</h3><p>Установите и откройте <b>AmneziaWG</b> на подключаемом устройстве. Нажмите в приложении кнопку добавления подключения и выберите сканирование QR-кода. Затем на этом сайте нажмите <b>«QR»</b> в карточке устройства и отсканируйте появившийся код.</p></details>"""
