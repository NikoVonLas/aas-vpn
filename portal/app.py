...
def cabinet(request: Request):
    ...
    create = "" if len(devices) >= user["device_limit"] else "<form class=device-form method=post action=/device><label>Название устройства<input name=name maxlength=40 placeholder='Телефон Пети' required></label><button>Добавить устройство</button></form>"
    guide = """<details class='card guide'><summary>Как подключиться</summary><h3>1. Сначала на этом сайте</h3><ol><li>В поле <b>«Название устройства»</b> напишите любое понятное название, например <b>Телефон Пети</b>.</li><li>Нажмите <b>«Добавить устройство»</b>. Ниже появятся кнопки QR и Файл.</li></ol><p class=muted>Название нужно только для удобства — можно написать что угодно.</p><h3>2. Затем в приложении</h3><ol><li>Установите <b>AmneziaWG</b> на телефон или компьютер, который хотите подключить.</li><li>Если сайт открыт на другом экране — нажмите <b>QR</b> и отсканируйте код через AmneziaWG.</li><li>Если сайт открыт на подключаемом устройстве — нажмите <b>Файл</b>, затем откройте скачанный файл через AmneziaWG.</li></ol></details>"""
