...
class SecurityPages:
    ...
    def finish(self, token):
        ...
        destination = '/security' if not session['ready'] or session['must_change'] else '/admin' if self.p.identities.owner(session['account_id']) else '/cabinet'
    ...
    def profile_actor(self, request, enrollment=False):
        ...
        if not actor['ready']:
            with self.p.auth_store.db() as con:
                row = con.execute('SELECT * FROM accounts WHERE id=?', (actor['account_id'],)).fetchone()
    ...
    def login_page(self, method: str='password'):
        ...
        return self.p.page('Вход', '<section class=card>' + nav + form + '</section>', phone_widget=method == 'phone')
    ...
    def lookup(self, identifier, method=None):
        with self.p.auth_store.db() as con:
            query = 'a.phone=?' if method == 'phone' else 'a.email=?' if method == 'email' else '(c.username=? OR a.phone=? OR a.email=?)'
    ...
    async def send_confirmation(self, request, method, identifier='', purpose='login'):
        ...
        if purpose == 'login':
            ...
            with self.p.auth_store.db() as con:
                account = dict(con.execute('SELECT * FROM accounts WHERE id=?', (actor['account_id'],)).fetchone())
            if purpose == 'second':
                ...
                address = self.p.phone_normalize(identifier) if method == 'phone' else identifier.strip().lower()
                if method == 'email' and ('@' not in address or len(address) > 254 or '\n' in address or ('\r' in address)):
    ...
    async def verify_confirmation(self, request: Request, key: str, code: str=Form('')):
        ...
        return self.finish(token) if token else RedirectResponse('/security', 303)
    ...
    def security_page(self, request: Request):
        ...
        body = self.p.admin_nav('/security') if actor['ready'] and self.p.admin_ok(request) else '<p><a href=/cabinet>Устройства</a></p>'
        if not actor['ready']:
    ...
    def passkey_finish(self, request: Request, key: str=Form(...), credential: str=Form(...)):
        ...
        try:
            if row['purpose'] == 'enroll-webauthn':
                ...
                return JSONResponse({'location': '/security'})
            with self.p.auth_store.db() as con:
    ...
    def modules(self, request: Request):
        ...
        body = self.p.admin_nav('/admin/login-methods')
    ...
    async def save_module(self, request: Request, provider_id: str):
        ...
        return RedirectResponse('/admin/login-methods', 303)
...
def register(portal):
    ...
    portal.app.add_api_route('/security', pages.security_page, methods=['GET'])
    ...
    portal.app.add_api_route('/admin/login-methods', pages.modules, methods=['GET'])
